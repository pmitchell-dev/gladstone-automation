...
- All shell scripts MUST reside in `/home/pi/scripts`.
- Data files (logs, registries) MUST remain in `/home/pi/scripts` or `/home/pi/printer_data`.
...
- **Architecture Awareness:** Scripts checking hardware (Temp/IP) must handle both RPi5 (ARM) and Laptop (x86) via `vcgencmd` check.
- **Error Handling:** Use `jq -r ".response // empty"` for API calls to prevent null-pointer crashes.

## 📅 Scheduling & Services
- **Crontab:** Never use `crontab -e`. Update the `pi_rebuild.sh` script and execute it to deploy new schedules.
- **Printer Sync:** The Brother Printer (192.168.50.56) is on a 4-hour scrape cycle (0, 8, 12, 16, 20).
- **Watchdog:** The `pi_services_manager.sh` is the master process monitor. Any new persistent service must be added to `services.registry`.
...
3. Run `sync` (alias for `pi_sync.sh`) to commit to GitHub.
4. Verify via `db` (alias for `pi-dashboard.sh`).
